...
- `V0.4.5` - Uses **XFeat*** pre-alignment before the global and bilateral stages: matches source/reference features, estimates a homography, crops to the valid overlap, and exposes an XFeat* debug viewport in the UI. The downstream pipeline then runs Neural Preset global matching followed by bilateral-grid affine transfer on the aligned pair.
