...
- `V0.3.5`: adds a fast MPS local diffuse-mood post-process after the global OT/SVD base. The global stage now hands off `base_intermediate` in OKLab to avoid an unnecessary RGB round trip, while `scripts/local_matching.py` transfers only smooth, bounded luma/hue/chroma maps from the reference and rejects residual-heavy detail, clipping, texture, artifacts, and unstable low-chroma hue.
